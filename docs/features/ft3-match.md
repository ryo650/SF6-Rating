...
- 1セットは最大5ゲームとする
- 1ラウンドの制限時間は60秒とする
- その他のゲーム内設定は原則としてSF6標準設定に従う。ただし、Round TimeのみSF6標準の99秒ではなく60秒とする
- SF6-RatingはRound Timeを含むSF6内の設定を自動検証しないため、60秒設定はHostと参加者が従う運用ルールとして扱う
...
- [ ] 1ラウンドの制限時間が60秒であることをルールとして明示する
- [ ] その他のゲーム内設定は原則SF6標準とし、Round Timeのみ標準の99秒ではなく60秒と明示する
- [ ] 60秒設定はSF6-Ratingが自動検証する値ではなく、Hostと参加者が従う運用ルールとして扱われる
