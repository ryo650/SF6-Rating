...
- [ ] **Purpose:** 実装開始時点のschema、package、Supabase公式contractを再確認し、planning driftを防ぐ。
...
- [ ] **Purpose:** Country / Broad Region、Profile constraints、Avatar、deletion / reclaim、rate-limitの永続契約を作る。
...
- [ ] **Purpose:** Supabase Auth userとapplication Profileを一対一に安全に紐付け、MR contractをversion化する。
...
- [ ] **Purpose:** Clientからnormalized valueやactorを偽装できない共通domain基盤を作る。
...
- [ ] **Purpose:** 後続sliceが同じdefault-deny認可とidempotency patternを使えるようにする。
...
- [ ] **Purpose:** Local / Previewで安全にsessionを確立・更新し、onboardingへ復帰できるようにする。
...
- [ ] **Purpose:** verified Emailだけでapplication onboardingを開始でき、passwordを安全にresetできるようにする。
...
- [ ] **Purpose:** MVP必須social providersを同じcallback / session contractへ接続する。
...
- [ ] **Purpose:** Unicode Usernameを正規化、一意化し、step単位でserverへ確定する。
...
- [ ] **Purpose:** Ownerだけが安全な静止Avatarを保存できるようにする。
...
- [ ] **Purpose:** UsernameとAvatarをja/en・mobileで理解可能に設定できるようにする。
...
- [ ] **Purpose:** searchable Player Name、canonical User Code、managed locationをprivate contractどおり保存する。
...
- [ ] **Purpose:** Player NameとUser Codeの用途差、公開範囲、地域選択を迷わず入力できるようにする。
...
- [ ] **Purpose:** self-reported rank / MRからserver-owned Starting Rating previewを一貫して計算する。
...
- [ ] **Purpose:** Profile公開、Rating、Placement、account active化の部分成功と二重初期化を防ぐ。
...
- [ ] **Purpose:** Starting Rating / Placementを理解して安全にonboardingを完了できるようにする。
...
- [ ] **Purpose:** Onboarding後の編集をcooldown、Active Match、privacy contractの内側で許可する。
...
- [ ] **Purpose:** Public / private項目、編集可否、次回変更可能日を明確に表示する。
...
- [ ] **Purpose:** active dependencyを壊さず削除意思を永続化し、新規matchmakingを止める。
...
- [ ] **Purpose:** PIIを消しつつMatch / Rating Historyを匿名参照で保持し、部分失敗を再試行可能にする。
...
- [ ] **Purpose:** 不可逆操作、blocking reason、pending stateを誤解なく扱えるようにする。
...
- [ ] **Purpose:** 実装者のblind spotをPhase 3へ持ち越さない。
...
- [ ] **Purpose:** Human往復を必要としない重大問題を一括で解消する。
